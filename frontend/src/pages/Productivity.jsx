...
export function TimeTracker() {
  ...
  const update=(id,patch)=>{ const nextTasks=tasks.map(t=>t.id===id?{...t,...patch}:t); saveDay(nextTasks); };
  ...
  return <Layout><Page><Hero title="24 Hours Time Tracker" subtitle="Day-wise planner with date. Complete Day 1 to open Day 2 automatically."><div className="scoreMini"><b>Day {day}</b><span>{selectedDate}</span><Progress value={focusScore}/></div></Hero>
    <Card title="Choose Day + Date" subtitle="Learning Calendar is now simplified into this 24h day-wise planner."><div className="row"><input style={{maxWidth:120}} type="number" min="1" max="90" value={day} onChange={e=>setDay(e.target.value)}/><input style={{maxWidth:180}} type="date" value={selectedDate} onChange={e=>saveDate(e.target.value)}/><button className="btn ghost" onClick={loadTemplate}>Load Day Template</button><button className="btn ghost" onClick={exportReport}>Export Daily Report</button><button className="btn cyan" onClick={completeDayAndOpenNext}>{isComplete?'Open Next Day':'Complete Day & Open Next Day'}</button></div></Card>
    <Card title={`Day ${day} Professional Routine`} subtitle="Daily required flow: Salesforce + DSA + System Design + Project + Interview + Revision."><div className="notificationStack"><div><b>01</b><span>2h Salesforce Core</span></div><div><b>02</b><span>1h DSA Practice</span></div><div><b>03</b><span>1h System Design</span></div><div><b>04</b><span>2h Project Proof</span></div><div><b>05</b><span>1h Interview Answer</span></div><div><b>06</b><span>1h Revision + Job Tracker</span></div></div></Card>
    <Card title="Quick Sprints" subtitle="Add focused blocks instantly"><div className="row"><button className="btn cyan" onClick={()=>quick('Salesforce Study',45,'45-min Salesforce sprint')}>+45m Salesforce</button><button className="btn cyan" onClick={()=>quick('Coding Practice',60,'1h DSA sprint')}>+1h DSA</button><button className="btn cyan" onClick={()=>quick('Reading / Theory',60,'1h System Design')}>+1h System Design</button><button className="btn cyan" onClick={()=>quick('Job Apply',30,'30-min job apply')}>+30m Job Apply</button></div></Card>
    <div className="statsGrid"><div className="stat"><span>TIME</span><p>Tracked Today</p><b>{fmt(total)}</b><small>Day {day} • {selectedDate}</small></div><div className="stat"><span>STUDY</span><p>Study Blocks</p><b>{tasks.filter(t=>productiveCats.includes(t.category)).length}</b><small>{fmt(productive)} productive</small></div><div className="stat"><span>JOB</span><p>Job Actions</p><b>{fmt(jobMin)}</b><small>Apply/follow-up</small></div><div className="stat"><span>FOCUS</span><p>Focus Score</p><b>{focusScore}%</b><small>{isComplete?'Day complete':'Day pending'}</small></div></div>
...
function getLocalStorageRows() {
  return Array.from({ length: localStorage.length }, (_, index) => {
    ...
    return { key, size: Math.round(value.length / 1024 * 10) / 10, preview: value.slice(0, 180) };
...
export function Backup() {
  ...
  return <Layout><Page><Hero title="Backup Center & Saved Data Viewer" subtitle="See exactly where app data is saved, preview it, and export it."/>
    <Card title="Where Your Data Is Stored" subtitle="Frontend saves immediately in browser localStorage. Backend SQLite is separate and works when backend sync/API is used.">
      ...
      <div className="row"><button className="btn cyan" onClick={exportBackup}>Export All Backup JSON</button><button className="btn ghost" onClick={refresh}>Refresh Saved Data</button><a className="btn ghost" href="http://127.0.0.1:8000/api/health" target="_blank" rel="noreferrer">Check Backend</a></div>
    </Card>
