...
const defaultBlocks = [
  ['09:00','10:00','Salesforce Study','Daily topic study',''], ['10:15','11:00','Hands-on Project','Practice implementation',''], ['18:30','19:15','Coding Practice','DSA practice',''], ['20:00','20:45','Interview Practice','Mock interview','']
...
export function TimeTracker() {
  const [day,setDay]=React.useState(()=>readStore('timeCurrentDay',1));
  const [all,setAll]=React.useState(()=>readStore('timeTasksByDay',{}));
  const tasks=all[day] || defaultBlocks;
  const [form,setForm]=React.useState({start:'09:00',end:'10:00',category:'Salesforce Study',task:'',notes:''});
  const saveDay=(items)=>{ const n={...all,[day]:items}; setAll(n); writeStore('timeTasksByDay',n); writeStore('timeCurrentDay',day); writeStore('timeTasks',items); };
  ...
  const total=tasks.reduce((a,t)=>a+minutesBetween(t.start,t.end),0); const doneMin=tasks.filter(t=>t.done).reduce((a,t)=>a+minutesBetween(t.start,t.end),0); const productive=tasks.filter(t=>productiveCats.includes(t.category)).reduce((a,t)=>a+minutesBetween(t.start,t.end),0); const jobMin=tasks.filter(t=>t.category==='Job Apply').reduce((a,t)=>a+minutesBetween(t.start,t.end),0); const focusScore=total?Math.round(productive/total*100):0;
  ...
  const exportReport=()=>downloadText(`day-${day}-time-report.txt`, `Day ${day} 24h Tracker\nTracked: ${fmt(total)}\nProductive: ${fmt(productive)}\nDone: ${fmt(doneMin)}\nFocus Score: ${focusScore}%\n\n`+tasks.map(t=>`${t.start}-${t.end} | ${t.category} | ${t.task} | ${t.done?'Done':'Pending'} | ${t.notes}`).join('\n'));
  return <Layout><Page><Hero title="24 Hours Time Tracker" subtitle=""><div className="scoreMini"><b>Day {day}</b><Progress value={focusScore}/></div></Hero>
    <Card title="Choose Day + Quick Sprints" subtitle="Har day ka record alag save hota hai"><div className="row"><input style={{maxWidth:140}} type="number" min="1" max="90" value={day} onChange={e=>setDay(Math.max(1,Math.min(90,Number(e.target.value)||1)))}/><button className="btn ghost" onClick={()=>saveDay(defaultBlocks)}>Load Template</button><button className="btn ghost" onClick={exportReport}>Export Daily Report</button><button className="btn cyan" onClick={()=>quick('Salesforce Study',45,'45-min Salesforce sprint')}>+45m Salesforce</button><button className="btn cyan" onClick={()=>quick('Coding Practice',30,'30-min DSA sprint')}>+30m DSA</button><button className="btn cyan" onClick={()=>quick('Job Apply',30,'30-min job apply')}>+30m Job Apply</button></div></Card>
    <div className="statsGrid"><div className="stat"><span>TIME</span><p>Tracked Today</p><b>{fmt(total)}</b><small>Day {day}</small></div><div className="stat"><span>STUDY</span><p>Study Blocks</p><b>{tasks.filter(t=>productiveCats.includes(t.category)).length}</b><small>{fmt(productive)} productive</small></div><div className="stat"><span>JOB</span><p>Job Actions</p><b>{fmt(jobMin)}</b><small>Apply/follow-up</small></div><div className="stat"><span>FOCUS</span><p>Focus Score</p><b>{focusScore}%</b><small>Productive / total</small></div></div>
    <Card title="Add Time Block" subtitle="Start/end time se duration auto calculate hoga"><div className="timeForm"><input type="time" value={form.start} onChange={e=>setForm({...form,start:e.target.value})}/><input type="time" value={form.end} onChange={e=>setForm({...form,end:e.target.value})}/><select value={form.category} onChange={e=>setForm({...form,category:e.target.value})}>{categories.map(c=><option key={c}>{c}</option>)}</select><input value={form.task} onChange={e=>setForm({...form,task:e.target.value})} placeholder="What are you doing?"/><textarea value={form.notes} onChange={e=>setForm({...form,notes:e.target.value})} placeholder="Notes / result / proof"/><button className="btn cyan" onClick={add}>Save Block</button></div></Card>
    <div className="grid2"><Card title="Category Graph" subtitle="Work ke accordingly time distribution"><div className="barGraph">{stats.map(s=><div className="barRow" key={s.cat}><b>{s.cat}</b><div><i style={{width:`${Math.max(4,total?s.min/total*100:0)}%`}}/></div><span>{fmt(s.min)}</span></div>)}</div></Card><Card title="24 Hour Heatmap" subtitle="Filled blocks show active work slots"><div className="heatmap">{Array.from({length:24},(_,h)=>{ const active=tasks.some(t=>{const st=Number(String(t.start).split(':')[0]); const en=Number(String(t.end).split(':')[0]); return h>=st && h<=en;}); return <span key={h} className={active?'on':''} title={`${h}:00`}>{h}</span>})}</div><div className="tip"><b>Learning Guidance</b><p>{productive<180?'Learning time is low today. Complete one 45-minute Salesforce sprint before entertainment.':'Good productive base today. Add one interview answer and one job follow-up.'}</p><p>Balance: Learning {fmt(productive)} • Career {fmt(jobMin)} • Other {fmt(Math.max(0,total-productive-jobMin))}</p></div></Card></div>
    <Card title="Saved Day Blocks" subtitle="Edit, delete, duplicate, done/undo"><div className="timeCards">{tasks.map(t=><div className={t.done?'timeCard done':'timeCard'} key={t.id}><div><b>{t.start} - {t.end}</b><span>{fmt(minutesBetween(t.start,t.end))}</span></div><select value={t.category} onChange={e=>update(t.id,{category:e.target.value,productive:productiveCats.includes(e.target.value)})}>{categories.map(c=><option key={c}>{c}</option>)}</select><input value={t.task} onChange={e=>update(t.id,{task:e.target.value})}/><textarea value={t.notes||''} onChange={e=>update(t.id,{notes:e.target.value})} placeholder="Proof / result / notes"/><div className="row"><button className="btn small" onClick={()=>update(t.id,{done:!t.done})}>{t.done?'Undo':'Done'}</button><button className="btn small ghost" onClick={()=>duplicate(t)}>Duplicate</button><button className="btn small danger" onClick={()=>saveDay(tasks.filter(x=>x.id!==t.id))}>Delete</button></div></div>)}</div></Card>
  </Page></Layout>;
}
...
function getLocalStorageRows() {
  ...
  }).filter(row => row.key && !['theme'].includes(row.key)).sort((a,b) => a.key.localeCompare(b.key));
}
...
export function Backup() {
  ...
  return <Layout><Page><Hero title="Backup Center & Saved Data Viewer" subtitle="See exactly where app data is saved, preview it, and export it."/>
    <Card title="Where Your Data Is Stored" subtitle="Frontend saves immediately in browser localStorage. Backend SQLite is separate and works when backend sync/API is used.">
      <div className="notificationStack"><div><b>01</b><span><strong>Browser Local Storage:</strong> most app data such as jobs, notes, answers, JD, resume, tracker data.</span></div><div><b>02</b><span><strong>Backend SQLite:</strong> E:\\sfdc-mentor-final-realtime-dashboard-dsa-fixed\\backend\\mentor_storage.sqlite3</span></div><div><b>03</b><span><strong>Export:</strong> use backup JSON before clearing browser data.</span></div></div>
      <div className="row"><button className="btn cyan" onClick={exportBackup}>Export All Backup JSON</button><button className="btn ghost" onClick={refresh}>Refresh Saved Data</button><a className="btn ghost" href="http://127.0.0.1:8000/api/items" target="_blank" rel="noreferrer">Open SQLite API Items</a><a className="btn ghost" href="http://127.0.0.1:8000/api/export" target="_blank" rel="noreferrer">Open SQLite Export</a></div>
    </Card>
    <Card title="Saved Browser Data" subtitle="All localStorage keys currently saved by the app.">
      <div className="tableWrap"><table><thead><tr><th>Key</th><th>Size KB</th><th>Preview</th><th>Action</th></tr></thead><tbody>{rows.map(row => <tr key={row.key}><td><b>{row.key}</b></td><td>{row.size}</td><td><small>{row.preview}</small></td><td><button className="btn small ghost" onClick={() => setSelectedKey(row.key)}>View</button></td></tr>)}</tbody></table></div>
    </Card>
    {selectedKey && <Card title={`Saved Data: ${selectedKey}`} subtitle="Preview of selected saved data."><textarea readOnly value={selectedValue} style={{minHeight:220}}/><div className="row"><button className="btn cyan" onClick={exportOne}>Export This Key</button><button className="btn ghost" onClick={() => setSelectedKey('')}>Close</button></div></Card>}
  </Page></Layout>;
}