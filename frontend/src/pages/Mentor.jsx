...
function formatPrompt(prompt) { return prompt?.length > 80 ? `${prompt.slice(0, 80)}...` : prompt; }
...
export function AIMentor() {
  ...
  function clearChat(){ setResponses([]); setQuestion(''); setToast('Chat cleared'); }
  function saveToNotes(item){ const notes=readStore('notes',[]); writeStore('notes',[...notes,{id:Date.now(),title:item.prompt || 'AI Mentor Answer',body:item.text,status:'Mentor',createdAt:new Date().toLocaleString()}]); setToast('Saved to Notes'); }
  function addToRevision(item){ const revision=readStore('revisionCalendar',[]); writeStore('revisionCalendar',[...revision,{id:Date.now(),topic:item.prompt || 'AI Mentor',note:item.text,next:'1/3/7/15/30 days',createdAt:new Date().toLocaleString()}]); setToast('Added to Revision Calendar'); }
  function markTopic(item,statusValue){ const weakStrong=readStore('weakStrong',{}); writeStore('weakStrong',{...weakStrong,[`mentor-${Date.now()}`]:statusValue}); setToast(`Marked ${statusValue}`); }
  function pinMessage(index){ setResponses(responses.map((m,i)=>i===index?{...m,pinned:!m.pinned}:m)); }
  function deleteMessage(index){ setResponses(responses.filter((_,i)=>i!==index)); setToast('Response deleted'); }
  ...
  function transform(text,type){
    ...
    setResponses([...responses,{role:'assistant',text:output,time:new Date().toLocaleString(),prompt:type,links:[]}]);
  }
  ...
  async function ask(){
    ...
    setResponses([assistantMessage]);
    setQuestion(''); setLoading(false); setToast('Answer generated. Prompt was not saved.');
  }

  return <Layout><Page><Hero title="AI Mentor Agent Pro" subtitle="Professional private mentor. Choose the mentor setup first, then ask your question. History is not saved automatically."><div className="mentorStatus"><span className={status.backend==='online'?'online':'offline'}>Backend {status.backend}</span><span className={status.ollama==='online'?'online':'offline'}>Ollama {status.ollama}</span><span className="online">Web links ready</span><button className="btn small ghost" onClick={checkStatus}>Refresh</button></div></Hero>
    {toast && <div className="toast">✅ {toast}</div>}
    <div className="mentorProSummary"><div><b>1</b><span>Select mentor setup</span></div><div><b>2</b><span>Ask one clear question</span></div><div><b>3</b><span>Save only useful answers</span></div></div>
    <div className="mentorCleanLayout mentorNoHistory">
      <section className="mentorMain mentorMainFull">
        <Card title="Mentor Controls" subtitle="Set answer style before asking. This keeps answers more accurate and interview-ready." className="mentorControlProCard"><div className="mentorControls mentorControlsPro"><select value={mode} onChange={e=>setMode(e.target.value)}>{mentorModes.map(x=><option key={x}>{x}</option>)}</select><select value={difficulty} onChange={e=>setDifficulty(e.target.value)}>{difficultyOptions.map(x=><option key={x}>{x}</option>)}</select><select value={interviewMode} onChange={e=>setInterviewMode(e.target.value)}>{['Technical Round','HR Round','Managerial Round','Project Round','Mock Interview'].map(x=><option key={x}>{x}</option>)}</select><select value={backendMode} onChange={e=>setBackendMode(e.target.value)}><option value="local">Local Only</option><option value="backend">FastAPI Backend</option><option value="ollama">Ollama Local AI</option></select><label className="toggleLine"><input type="checkbox" checked={useSavedData} onChange={e=>setUseSavedData(e.target.checked)}/> Ask from my saved data first</label></div></Card>
        <Card title="Ask Mentor" subtitle="Write one focused question. Your prompt will not be saved as chat history." className="mentorComposer mentorComposerTop mentorAskProCard"><textarea value={question} onChange={e=>setQuestion(e.target.value)} onKeyDown={e=>{if(e.key==='Enter'&&e.ctrlKey)ask();}} placeholder="Example: Explain Apex trigger with bulk-safe example and interview answer... Ctrl+Enter to ask"/><div className="row"><button className="btn cyan" disabled={loading} onClick={ask}>{loading?'Thinking...':'Ask Mentor'}</button><button className="btn ghost" onClick={()=>window.open(`https://www.google.com/search?q=${encodeURIComponent(question)}`,'_blank')}>Search Web</button><button className="btn ghost" onClick={()=>setQuestion('')}>Clear Input</button><button className="btn ghost" onClick={clearChat}>Clear Answer</button></div></Card>
        <div className="mentorMessages mentorAnswerOnly">{responses.length?responses.map((m,i)=><div key={i} className="msg aiMsg answerOnlyCard"><div className="msgHead"><b>AI Mentor</b><small>{m.source || ''} {m.time}</small>{m.pinned&&<span className="pill">Pinned</span>}</div>{m.prompt&&<div className="promptChip">Prompt used: {formatPrompt(m.prompt)}</div>}<pre>{m.text}</pre>{m.links?.length>0&&<div className="linkCards">{m.links.map(l=><a key={l.url || l} href={l.url || l} target="_blank">{l.title || 'Open Link'}</a>)}</div>}<div className="mentorActions"><button onClick={()=>copyText(m.text)} className="btn small ghost">Copy</button><button onClick={()=>downloadText('mentor-answer.txt',m.text)} className="btn small ghost">Export</button><button onClick={()=>saveToNotes(m)} className="btn small cyan">Save Notes</button><button onClick={()=>addToRevision(m)} className="btn small ghost">Add Revision</button><button onClick={()=>markTopic(m,'Weak')} className="btn small ghost">Weak</button><button onClick={()=>markTopic(m,'Strong')} className="btn small ghost">Strong</button><button onClick={()=>pinMessage(i)} className="btn small ghost">Pin</button><button onClick={()=>transform(m.text,'star')} className="btn small ghost">STAR</button><button onClick={()=>transform(m.text,'hinglish')} className="btn small ghost">Hinglish</button><button onClick={()=>transform(m.text,'simple')} className="btn small ghost">Simple English</button><button onClick={()=>transform(m.text,'scenario')} className="btn small ghost">Scenario Qs</button><button onClick={()=>transform(m.text,'followup')} className="btn small ghost">Follow-ups</button><button onClick={()=>deleteMessage(i)} className="btn small danger">Delete</button></div></div>):<div className="mentorEmpty mentorPremiumEmpty"><h2>Ask anything to your AI Mentor</h2><p>Select controls first, then ask a focused Salesforce, DSA, JD, project, resume or interview question.</p></div>}</div>
