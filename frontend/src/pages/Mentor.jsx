...
export function AIMentor() {
  ...
  return <Layout><Page><Hero title="AI Mentor Agent Pro" subtitle="Clean private mentor: your prompt is not shown as a chat bubble and history is not saved automatically."><div className="mentorStatus"><span className={status.backend==='online'?'online':'offline'}>Backend {status.backend}</span><span className={status.ollama==='online'?'online':'offline'}>Ollama {status.ollama}</span><span className="online">Web links ready</span><button className="btn small ghost" onClick={checkStatus}>Refresh</button></div></Hero>
    {toast && <div className="toast">✅ {toast}</div>}
    <div className="mentorCleanLayout mentorNoHistory">
      <section className="mentorMain mentorMainFull">
        <Card title="Ask Mentor" className="mentorComposer mentorComposerTop"><textarea value={question} onChange={e=>setQuestion(e.target.value)} onKeyDown={e=>{if(e.key==='Enter'&&e.ctrlKey)ask();}} placeholder="Ask Salesforce, DSA, JD, project, interview, error... Ctrl+Enter to ask"/><div className="row"><button className="btn cyan" disabled={loading} onClick={ask}>{loading?'Thinking...':'Ask Mentor'}</button><button className="btn ghost" onClick={()=>window.open(`https://www.google.com/search?q=${encodeURIComponent(question)}`,'_blank')}>Search Web</button><button className="btn ghost" onClick={()=>setQuestion('')}>Clear Input</button><button className="btn ghost" onClick={clearChat}>Clear Answer</button></div></Card>
