...
export function AIMentorPro() {
  ...
  return <Layout><Page><Hero title="AI Mentor Agent Pro" subtitle="Ask Salesforce questions and get professional answers." />
    <Card title="Ask Mentor"><textarea className="mentorProTextarea" value={question} onChange={e => setQuestion(e.target.value)} placeholder="Ask any Salesforce, Apex, LWC, Flow, SOQL, interview or project question..." /><div className="row"><button className="btn cyan" disabled={loading} onClick={ask}>{loading ? 'Thinking...' : 'Ask Mentor'}</button><button className="btn ghost" onClick={() => setQuestion('What is Salesforce?')}>Salesforce</button><button className="btn ghost" onClick={() => setQuestion('What is Apex?')}>Apex</button><button className="btn ghost" onClick={() => setQuestion('Explain Apex Trigger with project example')}>Trigger</button><button className="btn ghost" onClick={() => setQuestion('What is LWC?')}>LWC</button><button className="btn ghost" onClick={() => setQuestion('')}>Clear</button></div></Card>
    {answer ? <Card title="Professional Mentor Answer" subtitle={asked}><pre className="mentorProAnswer">{answer}</pre><div className="mentorActions"><button className="btn small ghost" onClick={() => navigator.clipboard?.writeText(answer)}>Copy</button><button className="btn small ghost" onClick={() => setAnswer('')}>Clear Answer</button>{backendLabel && <span className="pill">{backendLabel}</span>}</div></Card> : <Card title="Ready"><div className="promptGrid">{['What is Salesforce?','What is Apex?','Explain Apex Trigger with project example','What is LWC?','What is Flow?','What is SOQL?'].map(p => <button className="btn ghost" key={p} onClick={() => setQuestion(p)}>{p}</button>)}</div></Card>}
  </Page></Layout>;
}
