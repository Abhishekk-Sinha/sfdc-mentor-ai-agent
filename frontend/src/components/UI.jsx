...
export const navGroups = [
  { title: 'Start Here', items: [['Home Guide','/dashboard','🏠']] },
  ...
  { title: 'Practice', items: [['Question Bank','/practice','❓'], ['Scenario Practice','/scenarios','🧩'], ['Use Cases','/use-cases','🏗️'], ['Weekly Tests','/weekly-tests','🧪'], ['English Practice','/english','🗣️']] },
  ...
  { title: 'Data & Tools', items: [['24h Tracker','/time-tracker','⏱️'], ['Notes','/notes','📝'], ['Documents','/documents','📁'], ['Doubts','/doubts','❔'], ['Portfolio Editor','/portfolio-manager','🖼️'], ['All Tools','/more-tools','🧰'], ['Backup','/backup','💾']] },
];
...
const localRoutes = [
  ...
  { words: ['portfolio', 'photo', 'skill'], to: '/portfolio-manager', icon: '🖼️', group: 'Portfolio' }, { words: ['document', 'file', 'upload'], to: '/documents', icon: '📁', group: 'Documents' },
...
function FloatingActions() { const [open, setOpen] = React.useState(false); const actions = [['Save Note','/notes','📝'], ['Add Doubt','/doubts','❔'], ['Question Bank','/practice','❓'], ['Ask Mentor','/ai-mentor','🤖'], ['Add Job','/job-tracker','💼']]; return <div className="fabWrap">{open && <div className="fabMenu">{actions.map(([label,to,icon]) => <Link key={to} to={to} onClick={() => setOpen(false)}><span>{icon}</span>{label}</Link>)}</div>}<button className="fab" onClick={() => setOpen(!open)}>{open ? '×' : '+'}</button></div>; }
function MobileBottomNav() { const items = [['🏠','/dashboard','Home'], ['❓','/practice','Practice'], ['✨','/focus','Focus'], ['💼','/job-tracker','Jobs'], ['🤖','/ai-mentor','Mentor']]; const location = useLocation(); return <nav className="mobileBottomNav">{items.map(([icon,to,label]) => <Link key={to} className={location.pathname === to ? 'active' : ''} to={to}><span>{icon}</span><small>{label}</small></Link>)}</nav>; }
