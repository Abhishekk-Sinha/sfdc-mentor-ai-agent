...
const quickCommands = [
  ...
  { label: 'Update Portfolio Photo', to: '/portfolio-manager', icon: '🖼️', kind: 'Command' },
...
const localRoutes = [
  ...
  { words: ['document', 'file', 'upload'], to: '/documents', icon: '🗂️', group: 'Documents' },
...
function routeFor(key, text) {
  ...
  return localRoutes.find(r => r.words.some(w => hay.includes(w))) || { to: '/dashboard', icon: '🔎', group: 'Saved Data' };
}
function rowsFromStorage(key, parsed) {
  ...
  const add = (value, label, path) => {
    ...
    rows.push({ id: `${key}-${path}`, label: titleOf(value, label), snippet: text.slice(0, 260), to: route.to, icon: route.icon, group: route.group, kind: 'Saved Local Data', storageKey: key, searchable: `${key} ${label} ${text}`.toLowerCase() });
  };
  if (Array.isArray(parsed)) parsed.slice(0, 300).forEach((item, i) => add(item, `${key} #${i + 1}`, i));
  else if (parsed && typeof parsed === 'object') Object.entries(parsed).slice(0, 300).forEach(([subKey, value]) => add(value, subKey, subKey));
...
function getLocalSearchResults(query) {
  ...
  try {
    for (let i = 0; i < localStorage.length; i += 1) {
      ...
      if (!raw) continue;
      rowsFromStorage(key, safeJson(raw)).forEach(row => { if (row.searchable.includes(q)) results.push(row); });
    }
  } catch { return []; }
  const unique = new Map();
  results.forEach(r => { if (!unique.has(r.id)) unique.set(r.id, r); });
...
function ThemeSwitcher() {
  ...
  return <div className="themeSwitcher" aria-label="Theme selector">{['salesforce','glass','purple'].map(t => <button key={t} className={theme === t ? 'active' : ''} onClick={() => setTheme(t)} title={t}>{t === 'salesforce' ? '🔵' : t === 'glass' ? '🌙' : '🟣'}</button>)}</div>;
}

function CommandBar() {
  const navigate = useNavigate();
  const [q, setQ] = React.useState('');
  const toolMatches = [...quickCommands, ...allTools].filter(x => `${x.label} ${x.group || ''}`.toLowerCase().includes(q.toLowerCase())).slice(0, 6);
  const localMatches = React.useMemo(() => getLocalSearchResults(q), [q]);
  const open = (item) => { if (item.storageKey) localStorage.setItem('lastGlobalSearchResult', JSON.stringify(item)); setQ(''); navigate(item.to); };
  return <div className="commandBar globalSearchBar"><span>⌘</span><input value={q} onChange={e => setQ(e.target.value)} placeholder="Search saved notes, answers, doubts, jobs, projects, tools..." />{q && <div className="commandMenu localSearchMenu"><div className="searchSectionTitle">Saved local data</div>{localMatches.length ? localMatches.map(item => <button key={item.id} className="localSearchResult" onClick={() => open(item)}><span>{item.icon}</span><div><b>{item.label}</b><small>{item.kind} • {item.group} • key: {item.storageKey}</small><p>{item.snippet}</p></div></button>) : <div className="commandEmpty">No saved local data found for “{q}”. Save notes/answers/jobs first, then search here.</div>}<div className="searchSectionTitle">Tools and quick actions</div>{toolMatches.length ? toolMatches.map(item => <button key={`${item.label}-${item.to}`} onClick={() => open(item)}><span>{item.icon}</span><div><b>{item.label}</b><small>{item.kind || 'Tool'}{item.group ? ` • ${item.group}` : ''}</small></div></button>) : <div className="commandEmpty">No tool matched.</div>}</div>}</div>;
}

function FloatingActions() {
  const [open, setOpen] = React.useState(false);
  const actions = [['Save Note','/notes','📝'], ['Add Doubt','/doubts','❓'], ['Start Sprint','/time-tracker','⏱️'], ['Ask Mentor','/ai-mentor','🤖'], ['Update Photo','/portfolio-manager','🖼️'], ['Add Job','/job-tracker','💼']];
  return <div className="fabWrap">{open && <div className="fabMenu">{actions.map(([label,to,icon]) => <Link key={to} to={to} onClick={() => setOpen(false)}><span>{icon}</span>{label}</Link>)}</div>}<button className="fab" onClick={() => setOpen(!open)}>{open ? '×' : '+'}</button></div>;
}
...
export function Layout({ children }) {
  ...
  return <div className="appShell"><aside className="sidebar"><Link className="brand" to="/dashboard"><span>🤖</span><div><b>SFDC Mentor</b><small>AI Agent Portfolio</small></div></Link>{navGroups.map(group => <div className="navGroup" key={group.title}><h4>{group.title}</h4>{group.items.map(([label, to, icon]) => <NavLink key={to} to={to} className={({ isActive }) => isActive ? 'navItem active' : 'navItem'}><span>{icon}</span>{label}</NavLink>)}</div>)}</aside><main className="main"><header className="topbar premiumTopbar"><div className="topGreeting"><p>{greet}, Abhishek Kumar</p><b>Build skills. Track proof. Prepare interviews. Show portfolio.</b></div><CommandBar/><div className="row topActions"><ThemeSwitcher/><Link className="btn cyan" to="/mentor-route">Continue Learning</Link><Link className="btn ghost" to="/portfolio">Portfolio</Link></div></header>{children}<FloatingActions/><MobileBottomNav/></main></div>;
}
