...
export const navGroups = [
  { title: 'Start Here', items: [['Home Guide','/dashboard','HOME'], ['Learning Plan','/zero-to-hero','PLAN'], ['Advanced Tools','/premium-features','TOOLS']] },
  ...
  { title: 'Practice', items: [['English Practice','/english','ENG'], ['Question Bank','/practice','QNA'], ['Scenario Practice','/scenarios','SCN'], ['Use Cases','/use-cases','USE'], ['Weekly Tests','/weekly-tests','TEST']] },
...
const quickCommands = [
  { label: 'Open Learning Plan', to: '/zero-to-hero', icon: 'PLAN', kind: 'Command' },
  { label: 'Open Advanced Tools', to: '/premium-features', icon: 'TOOLS', kind: 'Command' },
...
const localRoutes = [
  { words: ['zerohero','architect','learningplan'], to: '/zero-to-hero', icon: 'PLAN', group: 'Learning Plan' },
  { words: ['automation','autotodayplan','autoweakrevisionqueue','autosundaytest'], to: '/dashboard', icon: 'AUTO', group: 'Silent Automation Data' },
  { words: ['premiumfeatures','advancedtools'], to: '/premium-features', icon: 'TOOLS', group: 'Advanced Tools' },
...
function FloatingActions() { const [open, setOpen] = React.useState(false); const actions = [['Save Note','/notes','NOTE'], ['Add Doubt','/doubts','DBT'], ['Learning Plan','/zero-to-hero','PLAN'], ['Ask Mentor','/ai-mentor','AI'], ['Add Job','/job-tracker','JOB']]; return <div className="fabWrap">{open && <div className="fabMenu">{actions.map(([label,to,icon]) => <Link key={to} to={to} onClick={() => setOpen(false)}><span>{icon}</span>{label}</Link>)}</div>}<button className="fab" onClick={() => setOpen(!open)}>{open ? 'x' : '+'}</button></div>; }
function MobileBottomNav() { const items = [['HOME','/dashboard','Home'], ['PLAN','/zero-to-hero','Plan'], ['FOCUS','/focus','Focus'], ['JOB','/job-tracker','Jobs'], ['AI','/ai-mentor','Mentor']]; const location = useLocation(); return <nav className="mobileBottomNav">{items.map(([icon,to,label]) => <Link key={to} className={location.pathname === to ? 'active' : ''} to={to}><span>{icon}</span><small>{label}</small></Link>)}</nav>; }
